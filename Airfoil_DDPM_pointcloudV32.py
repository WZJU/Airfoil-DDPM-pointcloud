...
class PointNetEncoder(nn.Module):
    def __init__(self, dim=2, dim_out=128):
        ...
        self.conv1 = torch.nn.Linear(dim, 64) #输入通道数、输出通道数、卷积核大小
        self.conv2 = torch.nn.Linear(64, 128)
        self.conv3 = torch.nn.Linear(128, dim_out)
    ...
    def forward(self, x):
        ...
        x = torch.max(x, 1, keepdim=True)[0] #沿着#[B, N, D]的第2个维度取maxpooling(N维度),[B, 1, D]
        return x
...
class Unet(nn.Module):
    ...
    def __init__(self, point_dim, context_dim, residual):
        ...
        self.encoder = PointNetEncoder(dim=2, dim_out=128)
        self.layers = ModuleList([
            ConcatSquashLinear(point_dim, 128, 128),
            ConcatSquashLinear(128, 256, 128),
            ConcatSquashLinear(256, 512, 128),
            ConcatSquashLinear(512, 256, 128),
