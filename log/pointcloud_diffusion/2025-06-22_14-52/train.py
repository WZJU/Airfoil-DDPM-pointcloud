...
from Airfoil_DDPM_pointcloudV33 import Unet
...
def main(opt):
    ...
    shutil.copy(os.path.join('Airfoil_DDPM_pointcloudV33.py'), str(exp_dir))
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optim, step_size=40, gamma=0.2, last_epoch=-1)

    time_step = opt.time_step
    alpha=1-torch.linspace(0.0001, 0.01, steps=time_step)
    ...
    if opt.data_path is not None:
        ...
        data_path = os.path.join("../data","train_data_pointcloudV2.npz")
    ...
    loaded_pointcloud[:, :, 1] *= 7
    loaded_pointcloud[:, :, 0] -= 0.5
    loaded_pointcloud[:, :, 0] *= 2
    ...
    augmenter = Dataloader.PointCloudAugmenter(
        translate_range=(-0.1, 0.1),  # 各维度在[-0.1, 0.1]内随机平移
